...
def project_points_to_image_plane(Qs, cam):
    ...
    Qs_homo = np.hstack((Qs, np.ones((n, 1))))
    qs_homo = cam.P @ Qs_homo.T
    qs = (qs_homo / qs_homo[-1, :])[:2, :]
...
def add_noise_to_image_points(qs, sigma):
    ...
    n = qs.shape[1]
    qs = qs + np.random.normal(0, sigma, (2, n))
...
def evaluate_reprojection_error(Qs, qs_l, qs_r, cams):
    ...
    # Project points
    qs_l_ = project_points_to_image_plane(Qs, cams.cam_l)
    qs_r_  = project_points_to_image_plane(Qs, cams.cam_r)
    
    # Calculate squared difference in each image plane
    l_error = (qs_l - qs_l_) * (qs_l - qs_l_)
    r_error = (qs_r - qs_r_) * (qs_r - qs_r_)
    
    l_error = np.sum(l_error)
    r_error = np.sum(r_error)
    
    error = (l_error + r_error) / (2*n)
...
def optimize_points(Qs, qs_l, qs_r, cams):
    ...
    for q_l, q_r, Q_init in zip(qs_l, qs_r, Qs):
        ...
        out = lmfit.minimize(objective_func, params, args=(q_l, q_r, cams.cam_l.P, cams.cam_r.P))
    
        
        Qs_opt[idx, 0] = out.params['x'].value
        Qs_opt[idx, 1] = out.params['y'].value
        Qs_opt[idx, 2] = out.params['z'].value
